...
- **GedOC** (`packages/gedoc/`) — OpenCode plugin + launcher. Constrained by what OpenCode's plugin API offers. Uses npm workspaces with nested `packages/plugin/` and `packages/launcher/`.
...
# GedOC-specific
npm -w packages/gedoc run build
...
- **Launcher isolation** (GedOC) — launcher uses `XDG_CONFIG_HOME=~/.config/gedoc`, never mutates user's normal OpenCode config

## Testing

- GedOC: Node.js built-in test runner (`node --test`)
...
- @packages/gedoc/CLAUDE.md
- @packages/gedoc/AGENTS.md
...
- **GedOC**: `gedoc-v*` tags → tarball + installers + GitHub release
