#!/usr/bin/env bash
# GedOC installer — works on macOS and Linux.
# Requires: curl, tar, Node.js >= 22
#
# Usage:
#   curl -fsSL https://github.com/edgyarmati/ged-mono/releases/latest/download/install.sh | bash
#   # or pin a version:
#   GEDOC_VERSION=0.4.0 curl -fsSL ... | bash
set -euo pipefail

REPO_SLUG="edgyarmati/ged-mono"
VERSION="${GEDOC_VERSION:-0.4.0}"
DATA_DIR="${GEDOC_DATA_DIR:-$HOME/.local/share/gedoc}"
BIN_DIR="${GEDOC_BIN_DIR:-$HOME/.local/bin}"
...
ASSET_NAME="gedoc-${VERSION}.tar.gz"
ASSET_URL="https://github.com/${REPO_SLUG}/releases/download/gedoc-v${VERSION}/${ASSET_NAME}"
SUMS_URL="https://github.com/${REPO_SLUG}/releases/download/gedoc-v${VERSION}/SHA256SUMS"
ARCHIVE="$TMP_DIR/${ASSET_NAME}"
SUMS_FILE="$TMP_DIR/SHA256SUMS"

info "Downloading GedOC v${VERSION}"
...
# The tarball contains gedoc-VERSION/...
SRC_DIR="$TMP_DIR/gedoc-${VERSION}"
if [ ! -d "$SRC_DIR" ]; then
  fail "Unexpected archive layout — expected gedoc-${VERSION}/ directory inside tarball"
fi
...
WRAPPER="${BIN_DIR}/gedoc"
cat > "$WRAPPER" <<WRAPPER_EOF
#!/usr/bin/env bash
exec node "${DATA_DIR}/lib/bin/gedoc.js" "\$@"
WRAPPER_EOF
chmod +x "$WRAPPER"
...
if ! node "${DATA_DIR}/lib/bin/gedoc.js" --check >/dev/null 2>&1; then
  # Fall back to syntax-only validation if the launcher check path changes.
  if ! node --check "${DATA_DIR}/lib/bin/gedoc.js" 2>/dev/null; then
    fail "Launcher script has syntax errors. Something went wrong with the install."
  fi
fi

info "GedOC v${VERSION} installed successfully"
printf '\n'

# ── Install mode: launcher (recommended) or plugin ──────────────────────────

INSTALL_MODE="${GEDOC_INSTALL_MODE:-}"

if [ -z "$INSTALL_MODE" ]; then
  cat <<'MODE_EOF'
GedOC can be set up in two ways:

  1) Launcher (recommended) — "gedoc" becomes a separate command.
     Your normal "opencode" setup is left untouched. GedOC runs in its
     own isolated config with its own managed OpenCode runtime.

  2) Plugin — GedOC is added to your existing OpenCode config.
...
if command -v rtk >/dev/null 2>&1; then
  ...
  printf 'GedOC can optionally use RTK, a CLI proxy that compresses bash output\n'
  printf '(git, ls, test runners, etc.) for 60-90%% token savings. It is not required.\n'
  printf '\n'
  if [ -z "${GEDOC_SKIP_RTK:-}" ]; then
    ...
    case "$_rtk_answer" in
      ...
      *)
        ...
        if brew install rtk 2>/dev/null; then
          ...
          warn "RTK installation failed. GedOC will work without it. Install later with: brew install rtk"
        fi
        ;;
    esac
  else
    info "Skipping RTK (GEDOC_SKIP_RTK is set). Install later with: brew install rtk"
  fi
else
...
if [ "$INSTALL_MODE" != "plugin" ] && ! command -v gedoc >/dev/null 2>&1; then
