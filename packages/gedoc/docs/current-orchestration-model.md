# Current GedOC Orchestration Model

GedOC uses **single-writer orchestration**.

The short version:

> The primary `gedoc` agent owns active-worktree writes and decisions. Optional subagents contribute intelligence only. They do not become parallel developers.

This document explains how the current system works for users and future maintainers.

## Why this model exists

Parallel writer agents are tempting, but they fragment implicit decisions: style, edge cases, tests, architecture, and product interpretation. GedOC instead keeps one coherent writer and injects extra intelligence around that writer.
...
- global: `~/.gedoc/settings.json`
- project override: `.gedoc/settings.json` — gitignored by default
...
GedOC does **not** implement writer subagents.
...
- The primary `gedoc` agent is the active-worktree writer by default.
