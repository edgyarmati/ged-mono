...
Build GedOC as an OpenCode plugin plus a thin `gedoc` launcher, preserving the Omni workflow while dropping the custom GedPi shell direction.

## Product shape

GedOC is not a custom terminal app and not a fork of OpenCode.

It is:
- an OpenCode plugin package
- a launcher command that starts OpenCode with GedOC-specific config and plugin loading
...
- create an GedOC-specific OpenCode config directory
- write a local plugin shim that loads `@gedoc/plugin`
- launch OpenCode with `OPENCODE_CONFIG`, `OPENCODE_CONFIG_DIR`, and `OPENCODE_CLIENT=gedoc`
...
1. **Agent instructions**
   - the default `gedoc` agent explains the mandatory workflow
2. **Custom tools**
...
- `gedoc` launches OpenCode with GedOC loaded without changing normal OpenCode behavior
- a project can be bootstrapped into `.ged/`
- the default GedOC agent knows and follows the Omni workflow
