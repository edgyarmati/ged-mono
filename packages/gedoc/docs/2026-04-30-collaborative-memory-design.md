...
The first GedOC memory model assumes one active task per checkout:
...
- some contributors may not use GedOC or GedPi;
...
GedOC needs to keep its useful durable-memory workflow without turning `.ged/` into a shared mutable bottleneck.
...
4. **Prefer explicit settings over hidden behavior.** Guardrail policy belongs in GedOC settings JSON, with global defaults and optional project-local overrides.
...
At the start of a change request, GedOC should eventually run a collaboration checkpoint:
...
Policy belongs in the effective GedOC settings JSON layer, not in committed `.ged/CONFIG.md`:
...
Settings resolution should follow the already-planned GedOC settings model:
...
Project-local settings are still settings, not durable Omni memory. They should be treated like the subagent/model settings layer: usually untracked unless the user/team intentionally versions them outside GedOC's defaults.

When blocked, the guard message should be explicit:

```text
GedOC guard: change requests should run on a feature branch, not main.
Create or switch to a branch, or set workflow.allowProtectedBranchChanges=true
in GedOC settings if this project intentionally allows direct protected-branch work.
```

## Non-Omni contributors

GedOC must assume `.ged/work/<id>/` can be missing or stale. Before planning or resuming work, it should inspect normal collaboration sources:
