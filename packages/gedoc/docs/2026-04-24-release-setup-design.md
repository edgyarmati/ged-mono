...
Ship a release-ready install path so a new user can run one setup command, then run `gedoc` and reach a working GedOC/OpenCode session with minimal manual steps.

## Problem

GedOC already has a working launcher and plugin, but the current onboarding flow is still contributor-oriented:

- users need the repo or workspace context
- there is no clear release installer story
- there is no single documented setup path that ends with `gedoc` being available on `PATH`

For a real release, GedOC needs a simple installation experience that matches the product boundary:

- OpenCode remains the host runtime
- GedOC remains the workflow layer
- the launcher stays thin
- setup gets the user to a working `gedoc` command quickly
...
```bash
curl -fsSL https://raw.githubusercontent.com/edgyarmati/gedoc/main/scripts/install.sh | bash
gedoc
```
...
```bash
git clone https://github.com/edgyarmati/gedoc
cd gedoc
./scripts/setup
gedoc
```
...
- gives GedOC a polished one-command headline install
...
- avoids turning GedOC into a full machine bootstrapper
- keeps OpenCode as the host product rather than something GedOC replaces

### Rejected alternatives

#### Fat installer that owns everything

Rejected for v1 because it would try to install and configure too much: Node, OpenCode, GedOC, provider auth, and system state. That is brittle and crosses the product boundary.
...
GedOC must be installable as a package that exposes an `gedoc` binary.

Requirements:

- package metadata must support global installation
- the launcher package must expose the `gedoc` bin cleanly
...
- delegate to `npx gedoc@latest setup`
...
Add a real `setup` subcommand to the published `gedoc` CLI.

Responsibilities:

- validate Node/npm prerequisites
- install `gedoc` globally so the steady-state `gedoc` command exists after bootstrap
- verify `gedoc` is available, or report the exact npm global bin path to add to `PATH`
- print the next step to start GedOC
...
- link the local launcher so `gedoc` is immediately runnable
...
The `gedoc` launcher should continue to own runtime setup on first launch.

It should keep doing the following:

- create isolated config under `~/.config/gedoc/opencode`
- write the plugin shim
- write `opencode.json`
- detect whether `opencode` is installed
- attempt best-effort OpenCode installation if missing
- launch OpenCode with GedOC-specific environment variables
...
4. check `npm --version`
5. install GedOC globally from npm
6. verify `gedoc` is on `PATH`
7. print success message and tell the user to run `gedoc`
...
- contributor setup
- troubleshooting for missing Node, missing `gedoc` on `PATH`, and first-run OpenCode install behavior
...
- `gedoc` not found after installation
...
- GedOC may install OpenCode on first run if it is missing
...
2. `./scripts/setup` leaves a runnable local `gedoc` command
3. installer script succeeds on a machine with Node/npm already present
4. first `gedoc` launch creates isolated config automatically
5. first `gedoc` launch handles missing OpenCode via best-effort install
...
2. type `gedoc`
3. have GedOC set up its isolated OpenCode config automatically
4. have OpenCode installed automatically if missing, or receive a clear manual fallback
5. reach a working GedOC session without editing config files manually
