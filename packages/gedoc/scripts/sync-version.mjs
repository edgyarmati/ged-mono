#!/usr/bin/env node
// Sync the GedOC version from the root package.json into every
...
const TARGETS = [
  ...
  },
  {
    ...
    transforms: [
      { pattern: /GEDOC_BINARY_VERSION\s*=\s*"([^"]+)"/u, label: "GEDOC_BINARY_VERSION" },
    ],
  },
  {
    file: "install.sh",
    transforms: [
      { pattern: /GEDOC_VERSION=(\d+\.\d+\.\d+)\s+curl/u, label: "doc-comment example" },
      { pattern: /VERSION="\$\{GEDOC_VERSION:-(\d+\.\d+\.\d+)\}"/u, label: "VERSION default" },
    ],
  },
  {
    file: "install.ps1",
    transforms: [
      { pattern: /\$env:GEDOC_VERSION\s*=\s*'(\d+\.\d+\.\d+)'/u, label: "doc-comment example" },
