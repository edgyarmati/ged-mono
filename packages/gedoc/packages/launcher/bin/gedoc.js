...
async function installManagedOpenCodeRuntime(version, homeDir) {
  ...
  if (!(await existsOnPath(npmCommand))) {
    process.stderr.write(
      "npm is required to install the managed OpenCode runtime. Install npm and rerun GedOC.\n",
    );
    process.exit(1);
  }
...
async function runSetup() {
  ...
  const npmCommand = isWindows() ? "npm.cmd" : "npm";
  if (!(await existsOnPath(npmCommand))) {
    process.stderr.write("npm is required for GedOC setup. Install npm and rerun `npx gedoc@latest setup`.\n");
    process.exit(1);
  }

  const target = getGedOCSetupTarget();
  process.stderr.write(`Installing ${target} globally so the gedoc command is available...\n`);
  const installed = await runCommand(npmCommand, ["install", "-g", target]);
  if (!installed) {
    process.stderr.write(
      `Failed to install ${target} globally. Try 'npm install -g ${target}' manually and rerun GedOC.\n`,
    );
    process.exit(1);
  }

  const gedocPresent = await existsOnPath(isWindows() ? "gedoc.cmd" : "gedoc");
  if (!gedocPresent) {
    ...
    process.stderr.write(
      `GedOC was installed, but the 'gedoc' command is not on PATH yet. Restart your shell or add ${binHint} to PATH.\n`,
    );
    process.exit(1);
  }

  process.stderr.write("GedOC setup complete. Next step: run `gedoc`.\n");
}
...
async function resolvePluginPath() {
  ...
  try {
    return import.meta.resolve("@gedoc/plugin");
  } catch {
    process.stderr.write(
      "Cannot find the GedOC plugin. Reinstall GedOC or run 'gedoc setup'.\n",
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  if (process.argv[2] === "--check" || process.argv[2] === "check") {
    ...
    process.stdout.write(`GedOC launcher OK; plugin resolved at ${pluginPath}\n`);
    return;
  }
  ...
  const explicitBin = process.env.GEDOC_OPENCODE_BIN ?? process.env.GEDCODE_OPENCODE_BIN;
  const release = getNativeLauncherReleaseMetadata();

  const opencodeBin = explicitBin || (await ensureManagedOpenCodeRuntime(release.opencodeVersion, homeDir));

  const pluginPath = await resolvePluginPath();
  const { configRoot, configPath } = await ensureGedOCConfig({
    homeDir,
