...
export const GEDOC_PACKAGE_NAME = "gedoc";
export const GEDOC_SETUP_TARGET = `${GEDOC_PACKAGE_NAME}@latest`;

// Restrict GEDOC_SETUP_TARGET to the gedoc package (with optional
...
const SAFE_SETUP_TARGET = /^gedoc(?:@[A-Za-z0-9._\-+]{1,128})?$/u;
...
export function getGedOCSetupTarget() {
  const fromEnv = process.env.GEDOC_SETUP_TARGET ?? process.env.GEDCODE_SETUP_TARGET;
  if (fromEnv && fromEnv.length > 0) {
    if (!isSafeSetupTarget(fromEnv)) {
      throw new Error(
        `GEDOC_SETUP_TARGET must be of the form "gedoc" or "gedoc@<version>"; refusing to use ${JSON.stringify(fromEnv)}.`,
      );
    }
    return fromEnv;
  }
  return GEDOC_SETUP_TARGET;
}

export function getXdgConfigHome(homeDir = os.homedir()) {
  return path.join(homeDir, ".config", "gedoc");
}

export function getGedOCDataRoot(homeDir = os.homedir(), platform = process.platform, env = process.env) {
  if (platform === "darwin") {
    return path.join(homeDir, "Library", "Application Support", "gedoc");
  }
  if (platform === "win32") {
    return path.join(env.LOCALAPPDATA || path.join(homeDir, "AppData", "Local"), "GedOC");
  }
  return path.join(env.XDG_DATA_HOME || path.join(homeDir, ".local", "share"), "gedoc");
}

export function getManagedOpenCodeRoot(homeDir = os.homedir(), platform = process.platform, env = process.env) {
  return path.join(getGedOCDataRoot(homeDir, platform, env), "runtimes", "opencode");
}
...
export function getNativeLauncherReleaseMetadata(platform = process.platform, arch = process.arch) {
  return {
    launcherVersion: GEDOC_BINARY_VERSION,
    opencodeVersion: OPENCODE_VERSION_TARGET,
    assetName: getLauncherAssetName(platform, arch),
    assetUrl: getVersionedReleaseAssetUrl(platform, arch, GEDOC_BINARY_VERSION),
  };
}
...
// Both files we write under ~/.config/gedoc/opencode/ are in a
...
export async function ensureGedOCConfig({
  homeDir = os.homedir(),
  pluginEntry,
}) {
  ...
  const pluginShimPath = path.join(pluginDir, "gedoc-plugin.js");
  ...
  const shimSource = `export { GedOCPlugin, default } from ${JSON.stringify(resolvedEntry)};\n`;
