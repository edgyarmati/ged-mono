...
import { GedOCPlugin, ensureOmniDir, setOmniMode, branchNameToWorkId, readCurrentGitBranch } from "../src/index.ts";
...
async function withTempDir(run: (dir: string) => Promise<void>) {
  const dir = await mkdtemp(path.join(os.tmpdir(), "gedoc-hook-test-"));
  try {
    await run(dir);
  } finally {
...
async function buildHook(directory: string): Promise<ToolExecuteBefore> {
  const hooks = await GedOCPlugin(
    { directory } as unknown as PluginInput,
  );
  const hook = hooks["tool.execute.before"];
  if (!hook) throw new Error("GedOCPlugin did not register tool.execute.before");
...
const V2_CLARIFICATION = {
  clarification: {
    ...
    evidence: {
      ...
      users: "Engineers working on GedOC",
...
async function writeRealPlanning(directory: string) {
  ...
  await writeFile(
    ...
    [
      ...
      "## Problem",
      "",
      "The tool.execute.before hook must reject mutating tool calls when the durable planning artifacts are still placeholder bootstrap content. Writing source files before there is a real spec, task list, and test list defeats GedOC's plan-before-edit discipline.",
      "",
      "## Requested Behavior",
      "",
...
test("tool.execute.before rejects write/edit when planning artifacts are placeholders", async () => {
  await withTempDir(async (dir) => {
    ...
    await assert.rejects(
      ...
      /GedOC guard: before editing source files/,
    );
    await assert.rejects(
      () => hook(
        { tool: "edit" },
...
test("tool.execute.before rejects mutating bash commands when planning artifacts are placeholders", async () => {
  await withTempDir(async (dir) => {
    ...
    await assert.rejects(
      ...
      /GedOC guard: before editing source files or running mutating shell commands/,
    );
    await assert.rejects(
      () => hook(
        { tool: "bash" },
