# GedOC installer for Windows.
# Requires: PowerShell 5.1+, Node.js >= 22
#
# Usage:
#   irm https://github.com/edgyarmati/ged-mono/releases/latest/download/install.ps1 | iex
#   # or pin a version:
#   $env:GEDOC_VERSION = '0.4.0'; irm ... | iex

$ErrorActionPreference = 'Stop'

$RepoSlug = 'edgyarmati/ged-mono'
$Version = if ($env:GEDOC_VERSION) { $env:GEDOC_VERSION } else { '0.4.0' }
$DataDir = if ($env:GEDOC_DATA_DIR) {
  $env:GEDOC_DATA_DIR
} elseif ($env:LOCALAPPDATA) {
  Join-Path $env:LOCALAPPDATA 'GedOC'
} else {
  Join-Path $HOME '.local\share\gedoc'
}
$BinDir = if ($env:GEDOC_BIN_DIR) {
  $env:GEDOC_BIN_DIR
} elseif ($env:LOCALAPPDATA) {
  Join-Path $env:LOCALAPPDATA 'GedOC\bin'
} else {
...
$TempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("gedoc-install-" + [System.Guid]::NewGuid())
New-Item -ItemType Directory -Force -Path $TempDir | Out-Null

try {
  $AssetName = "gedoc-$Version.tar.gz"
  $AssetUrl = "https://github.com/$RepoSlug/releases/download/gedoc-v$Version/$AssetName"
  $SumsUrl = "https://github.com/$RepoSlug/releases/download/gedoc-v$Version/SHA256SUMS"
  $ArchivePath = Join-Path $TempDir $AssetName
  $SumsPath = Join-Path $TempDir 'SHA256SUMS'

  Write-Host "==> Downloading GedOC v$Version"
  try {
  ...
  $SrcDir = Join-Path $TempDir "gedoc-$Version"
  if (-not (Test-Path $SrcDir)) {
    Write-Error "Unexpected archive layout - expected gedoc-$Version/ inside tarball"
    exit 1
  }
  ...
  # .cmd wrapper
  $CmdWrapper = Join-Path $BinDir 'gedoc.cmd'
  $EntryJs = Join-Path $LibDir 'bin\gedoc.js'
  @"
@echo off
node "$EntryJs" %*
"@ | Set-Content -Path $CmdWrapper -Encoding ASCII
  ...
  # PowerShell wrapper (so `gedoc` also works in PowerShell)
  $Ps1Wrapper = Join-Path $BinDir 'gedoc.ps1'
  @"
node "$EntryJs" @args
"@ | Set-Content -Path $Ps1Wrapper -Encoding ASCII
  ...
  Write-Host ''
  Write-Host "==> GedOC v$Version installed successfully"
  Write-Host ''

  # ── Install mode: launcher (recommended) or plugin ────────────────────────

  $InstallMode = $env:GEDOC_INSTALL_MODE
  if (-not $InstallMode) {
    Write-Host 'GedOC can be set up in two ways:'
    Write-Host ''
    Write-Host '  1) Launcher (recommended) -- "gedoc" becomes a separate command.'
    Write-Host '     Your normal "opencode" setup is left untouched. GedOC runs in its'
    Write-Host '     own isolated config with its own managed OpenCode runtime.'
    Write-Host ''
    Write-Host '  2) Plugin -- GedOC is added to your existing OpenCode config.'
  ...
  if ($InstallMode -eq 'plugin') {
    ...
    Write-Host ''
    Write-Host '  {'
    Write-Host '    "default_agent": "gedoc",'
  ...
  if ($rtkCmd) {
    ...
    Write-Host '    GedOC works fine without it. If RTK becomes available for Windows,'
  ...
  if ($InstallMode -ne 'plugin') {
    $gedocCmd = Get-Command gedoc -ErrorAction SilentlyContinue
    if (-not $gedocCmd) {
