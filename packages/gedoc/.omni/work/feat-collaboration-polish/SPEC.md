...
GedOC now enforces clarification, skill-fit, branch-scoped planning, verification, and commits, but it does not yet encode several engineering-practice workflows that improve implementation quality inside each slice. Matt Pocock's public skills highlight useful patterns for TDD, disciplined diagnosis, documentation-aware clarification, and architecture review. GedOC should adapt those concepts into versioned, tested, GedOC-native workflow resources.

## Requested Behavior

Implement bundled GedOC workflow support for:
...
- Adapt concepts into bundled GedOC resources; do not install or depend on external skills at runtime.
...
GedOC's launcher pins the managed upstream OpenCode runtime target at `1.14.25`, while npm reports the latest `opencode-ai` version as `1.14.30`. New GedOC-managed installs should use the latest tested OpenCode runtime without changing GedOC's own package version.

### Requested Behavior

- Update the launcher OpenCode target version from `1.14.25` to `1.14.30`.
- Keep GedOC package versions at `0.3.0`.
- Update tests, docs, and changelog only where they explicitly assert or describe the target version.

### Constraints

- This is a dependency/runtime target bump, not an GedOC release version bump.
...
The current `feat/collaboration-polish` branch diverged before main merged the subagent workflow PR. The branch lacks the main commits for GedOC settings primitives, optional omni subagents, and the `/ged-agents` setup command.
...
- Keep OpenCode as the host and GedOC as the workflow layer.
...
- Docs explain how Repo Prompt-style discovery/curation/handoff and Cognition-style single-writer review loops map to GedOC.
...
- Cover the primary `gedoc` agent, optional intelligence subagents, `/clean-context-review`, `/ged-agents`, and the reason writer subagents are not implemented.
...
The current code still exposes a legacy writer subagent even though GedOC's intended model is single-writer orchestration with intelligence-only subagents. Keeping that compatibility placeholder creates confusion and implies writer subagents remain in scope.
...
- Keep the primary `gedoc` agent as the only active-worktree writer.
...
- `GedOCSettings.agents` includes an `options` field.
- `buildSubagentConfig` merges per-agent options into the returned agent config.
- `gedoc_update_agents_settings` accepts an `options` parameter and persists it to settings.
- `gedoc_agents_status` reports effective per-agent options.
